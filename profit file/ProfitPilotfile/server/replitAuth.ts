  ...
  async () => {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        console.error(`[Auth] OIDC discovery attempt ${attempt} failed:`, error.message);
        
        if (attempt === maxRetries) {
          console.error(`[Auth] All OIDC discovery attempts failed. Check network connectivity and ISSUER_URL.`);
          throw error;
        }
